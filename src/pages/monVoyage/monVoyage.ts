...
@Component({
  selector: 'page-monVoyage',
  templateUrl: 'monVoyage.html',
})
export class MonVoyagePage {


  departure_date_time: string;
  arrival_date_time: string;
  sections: ChangmentModel[] = [];
  date: Date;
  ...
  public getVoyage(){
    ...
    this.sqliteService.getChangments().subscribe(val=> {
      ...
      this.departure_date_time = this.sections[0].departure_date_time;

      console.log("longueur section= " + this.sections.length);
      this.arrival_date_time = this.sections[this.sections.length - 1].arrival_date_time;

      console.log("mon voyage date de départ: " + this.sections[0].departure_date_time);
      console.log("mon voyage date de arrivee: " + this.sections[0].arrival_date_time);

      console.log(this.dateDisplay(this.departure_date_time));
      console.log(this.durationDisplay(this.departure_date_time, this.arrival_date_time));
      console.log(this.timeLeft(this.arrival_date_time));
      console.log(this.changeCounter(this.sections));
      console.log(this.durationMinutes(this.sections[0].duration));
    });
  ...
  public share(){
    ...
    console.log(this.TimeDisplay(this.departure_date_time));
      ...
      buttons: [
        {
          ...
          handler: () => {
            this.sharing.shareVia("com.facebook.orca",'Mon train part à  ' +this.TimeDisplay(this.departure_date_time));
          }
        },
        {
          text: 'Sms',
          handler: () => {
            for(let entry of this.contacts){
              this.sms.send(entry.phoneNumber , 'Mon train part à  ' +this.TimeDisplay(this.departure_date_time));
            }
          }
        }
      ]
